...
class SystemDriftMonitor {
    ...
    compareAgainstBaseline(recentMetrics) {
        ...
        const safeShortTerm = recentMetrics.short_term || {};
        const safeMediumTerm = recentMetrics.medium_term || {};
        const safeLongTerm = recentMetrics.long_term || {};
        ...
        return {
            ...
            accuracy_drift: this.calculateMetricDrift(safeBaseline.accuracy_rate || 0, safeShortTerm.accuracy_rate || 0),
            confidence_drift: this.calculateMetricDrift(safeBaseline.avg_confidence || 0, safeShortTerm.avg_confidence || 0),
            governance_drift: this.calculateMetricDrift(safeBaseline.governance_rate || 0, safeShortTerm.governance_rate || 0),
            bias_drift: this.calculateMetricDrift(safeBaseline.bias_rate || 0, safeShortTerm.bias_rate || 0),
            decision_latency_drift: this.calculateMetricDrift(safeBaseline.avg_decision_time || 0, safeShortTerm.avg_decision_time || 0),
            
            // Trend analysis (short vs medium vs long term)
            trend_analysis: {
                accuracy_trend: this.calculateTrend(
                    safeShortTerm.accuracy_rate || 0,
                    safeMediumTerm.accuracy_rate || 0,
                    safeLongTerm.accuracy_rate || 0
                ),
                confidence_trend: this.calculateTrend(
                    safeShortTerm.avg_confidence || 0,
                    safeMediumTerm.avg_confidence || 0,
                    safeLongTerm.avg_confidence || 0
                ),
                governance_trend: this.calculateTrend(
                    safeShortTerm.governance_rate || 0,
                    safeMediumTerm.governance_rate || 0,
                    safeLongTerm.governance_rate || 0
                ),
                bias_trend: this.calculateTrend(
                    safeShortTerm.bias_rate || 0,
                    safeMediumTerm.bias_rate || 0,
                    safeLongTerm.bias_rate || 0
                )
            }
        };
    }
    ...
    calculateDriftScore(driftAnalysis) {
        ...
        return {
            drift_score: driftScore.drift_score || driftScore,
            max_drift: driftScore.max_drift || maxDrift,
            weighted_drifts: driftScore.weighted_drifts || Object.fromEntries(
    ...
    detectDriftPatterns(driftAnalysis) {
        ...
        // Handle missing drift analysis gracefully
        const safeDriftAnalysis = driftAnalysis || {};
        const safeTrendAnalysis = safeDriftAnalysis.trend_analysis || {};
        
        // Pattern 1: Consistent negative drift
        const numericValues = Object.values(safeDriftAnalysis).filter(value => typeof value === 'number');
        if (numericValues.every(value => value < -0.1)) {
            patterns.push({
                type: 'consistent_negative_drift',
                severity: 'high',
                description: 'All metrics trending negative',
                affected_metrics: Object.keys(safeDriftAnalysis)
            });
        }
        
        // Pattern 2: Governance overuse
        if (safeDriftAnalysis.governance_drift > 0.3) {
            patterns.push({
                type: 'governance_overuse',
                severity: 'medium',
                description: 'High governance activation rate',
                value: safeDriftAnalysis.governance_drift
            });
        }
        
        // Pattern 3: Bias overreliance
        if (safeDriftAnalysis.bias_drift > 0.2) {
            patterns.push({
                type: 'bias_overreliance',
                severity: 'medium',
                description: 'High execution bias activation rate',
                value: safeDriftAnalysis.bias_drift
            });
        }
        
        // Pattern 4: Confidence collapse
        if (safeDriftAnalysis.confidence_drift < -0.3 && safeDriftAnalysis.accuracy_drift < -0.2) {
            patterns.push({
                type: 'confidence_collapse',
                severity: 'high',
                description: 'Confidence and accuracy both declining',
                confidence_drift: safeDriftAnalysis.confidence_drift,
                accuracy_drift: safeDriftAnalysis.accuracy_drift
            });
        }
        
        // Pattern 5: Action pattern shift
        const trendChanges = [
            safeTrendAnalysis.accuracy_trend || 0,
            safeTrendAnalysis.governance_trend || 0,
            safeTrendAnalysis.bias_trend || 0
        ];
    ...
    calculateSystemHealth(driftScore) {
        ...
        const actualDriftScore = typeof driftScore === 'object' ? (driftScore.drift_score || driftScore) : driftScore;
        
        // Health factors
        const driftHealth = Math.max(0, 1 - actualDriftScore); // Invert drift score for health
    ...
    updateDriftTracking(driftScore, driftAnalysis, driftPatterns, timestamp) {
        const actualDriftScore = typeof driftScore === 'object' ? (driftScore.drift_score || driftScore) : driftScore;
        this.currentDriftScore = actualDriftScore;
        ...
        const record = {
            ...
            drift_analysis: driftAnalysis,
            drift_patterns: driftPatterns,
            system_health: this.calculateSystemHealth(actualDriftScore)
        };
        ...
        console.log(`[SYSTEM DRIFT MONITOR] Drift score: ${actualDriftScore.toFixed ? actualDriftScore.toFixed(3) : actualDriftScore} | Health: ${record.system_health ? (record.system_health.healthy ? 'healthy' : 'unhealthy') : 'unknown'}`);
    }
    ...
    generateDriftAlerts(driftScore, driftPatterns) {
        ...
        if (actualDriftScore > this.alertThresholds.critical) {
            alerts.push({
                severity: 'critical',
                message: `Critical system drift detected: ${(actualDriftScore * 100).toFixed ? (actualDriftScore * 100).toFixed(1) : actualDriftScore * 100}% drift`,
                recommendations: this.generateCriticalRecommendations(driftPatterns),
                timestamp: new Date().toISOString()
            });
        } else if (actualDriftScore > this.alertThresholds.moderate) {
            alerts.push({
                severity: 'moderate',
                message: `System drift detected: ${(actualDriftScore * 100).toFixed ? (actualDriftScore * 100).toFixed(1) : actualDriftScore * 100}% drift`,
                recommendations: this.generateModerateRecommendations(driftPatterns),
                timestamp: new Date().toISOString()
            });
        } else if (actualDriftScore > this.alertThresholds.minor) {
            alerts.push({
                severity: 'minor',
                message: `Minor system drift detected: ${(actualDriftScore * 100).toFixed ? (actualDriftScore * 100).toFixed(1) : actualDriftScore * 100}% drift`,
