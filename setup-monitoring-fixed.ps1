...
try {
  Write-SetupLog "Setting up HYDI monitoring scheduler (fixed)"
  ...
  # Create new task with fixed duration
  $scriptPath = Join-Path $PSScriptRoot "complete-monitoring.ps1"
  $action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-ExecutionPolicy Bypass -File `"$scriptPath`""
  
  # Fixed trigger - use 1 year duration instead of MaxValue
  ...
  $taskState = Get-ScheduledTaskInfo -TaskName "HYDI Health Monitor"
  Write-SetupLog "Task status: $($taskState.State)"
