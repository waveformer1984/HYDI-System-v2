...
const logger = require('../../lib/structured-logger').child({ component: 'StripeWebhook' });
...
async function handleStripeWebhook(req, res) {
  ...
  try {
    ...
    logger.info('Webhook signature verification failed', { error: err.message });
  ...
  // Already processed
  if (!eventId) {
    logger.info('Idempotency: event already processed', { eventId: event.id });
  ...
  if (!gateStatus.authorized) {
    ...
    logger.info('CASCADE action: event dropped', { eventId: event.id, reason: gateStatus.reason });
  ...
  logger.info('CASCADE passed: queuing event for processing', { eventType: event.type });
  ...
  try {
    ...
    logger.error('Queue fail: failed to queue event', { error: err.message });
