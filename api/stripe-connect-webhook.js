...
const logger = require('../lib/structured-logger').child({ component: 'api/stripe-connect-webhook' });
...
async function handler(req, res) {
  ...
  logger.info('Webhook event received', { type: event.type, eventId: event.id });
...
async function handlePaymentIntentSucceeded(paymentIntent) {
  ...
  logger.info('Payment intent succeeded', { revenueStream, gross, net });
...
function determineRevenueStream(paymentIntent) {
  ...
  logger.warn('Cannot determine revenue stream, defaulting to galactic_bytes', { paymentIntentId: paymentIntent.id });
