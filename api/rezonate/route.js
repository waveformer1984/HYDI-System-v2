...
const logger = require('../../lib/structured-logger').child({ component: 'api/rezonate/route' });
  ...
  try {
    switch (action) {
      ...
      case 'list_tracks': {
        ...
        if (error) {
          logger.error('list_tracks error', { error });
          return res.status(500).json({ data: null, error: error.message });
        }
        return res.status(200).json({ data, error: null });
      }
      ...
      case 'add_track': {
        ...
        const { data, error } = await supabase
          ...
          .single();
        if (error) {
          logger.error('add_track error', { error });
          return res.status(500).json({ data: null, error: error.message });
        }
        return res.status(201).json({ data, error: null });
      }
      ...
      case 'dispatch_task': {
        ...
        const { data, error } = await supabase
          ...
          .single();
        if (error) {
          logger.error('dispatch_task error', { error });
          return res.status(500).json({ data: null, error: error.message });
        }
        return res.status(201).json({ data, error: null });
      }
