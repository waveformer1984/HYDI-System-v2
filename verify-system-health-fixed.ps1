...
try {
  ...
  foreach ($k in $requiredLocal) {
    if ($localRaw -match "(?m)^\s*$k\s*=\s*.+\s*$") {
      ...
      $missingLocal += $k
      Write-Host "ERROR: Local: $k missing/empty" -ForegroundColor Red
    }
  }
...
Write-Host "OK: Local environment: ALIGNED" -ForegroundColor Green
Write-Host "WARN: Vercel CLI: ENCODING ISSUES" -ForegroundColor Yellow
Write-Host "OK: System health: OPERATIONAL" -ForegroundColor Green

Write-Host "`nSTATUS: SYSTEM OPERATIONAL (Vercel CLI needs manual verification)" -ForegroundColor Green
