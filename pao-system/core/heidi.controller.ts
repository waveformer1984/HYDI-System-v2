...
import { UnifiedKnowledgeBase } from '../../modules/ucl-knowledge-base';
...
/**
 * HEIDI - Executive Orchestrator
 * 
 * Mission: Decide WHO does WHAT, enforce system efficiency, escalate high-risk decisions
 * 
 * Core Principles:
...
export class HeidiController {
  ...
  /**
   * Initialize task routing matrix - WHO does WHAT
   */
  ...
  private setupEventHandlers(): void {
    this.eventBus.subscribe({
      ...
      event_types: ['*'], // Listen to all events
      handler: async (event) => {
  ...
  /**
   * Main event processing loop - THE HEIDI BRAIN
   */
  private async processEvent(event: any): Promise<void> {
    ...
    // Step 1: Check for redundancy - prevent duplicate work
    ...
    // Step 3: Approval escalation if needed
  ...
  /**
   * Check for redundant tasks - PREVENT DUPLICATE WORK
   */
  private isRedundantTask(event: any): boolean {
    const taskKey = `${event.type}_${JSON.stringify(event.payload)}`;
    
    // Check if similar task is already active
  ...
  /**
   * Assign agents to directive - WHO does WHAT
   */
  private assignAgents(directive: HeidiDirective): string[] {
    const routingAgents = this.taskRoutingMatrix.get(directive.task_type) || [];
    ...
    // Check agent availability and load balance
    return this.validateAgentAvailability(assignedAgents);
  }

  /**
   * Validate agent availability and load balance
   */
  private validateAgentAvailability(agents: string[]): string[] {
    ...
    if (availableAgents.length === 0) {
      ...
      return agents; // Return original agents, will handle failure
    }
  ...
  private async routeToAgent(agentId: string, directive: HeidiDirective): Promise<void> {
    ...
    try {
      ...
      this.handleTaskFailure(directive, error.message);
    }
  }
  ...
  /**
   * Generate clear task description
   */
  ...
  /**
   * Force task completion (for testing/debug)
   */
  ...
  async start(): Promise<void> {
    ...
    console.log('[HEIDI] Mission: Decide WHO does WHAT, enforce efficiency, escalate risks');
    console.log('[HEIDI] Behavioral Directives: Speed + correctness, no redundancy, maintain coherence');
    console.log('');
    
    // Start event processing loop
    while (this.running) {
      try {
        // Event processing is handled by event handlers
