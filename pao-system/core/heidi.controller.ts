...
// ── Local event schema (matches EventBus event shape) ────────────────────────
...
export class HeidiController {
  ...
  private activeTasks = new Map<string, HeidiDirective>()
  private completedTasks: HeidiDirective[] = []
  private failedTasks: HeidiDirective[] = []
  private systemMetrics: SystemMetrics
  private taskRoutingMatrix: Map<string, string[]>
  private conflictResolutionHistory: ConflictRecord[] = []
  ...
  private initializeRoutingMatrix(): void {
    ...
    this.taskRoutingMatrix = new Map(entries)
  }
  ...
  private setupEventHandlers(): void {
    this.eventBus.subscribe({
      ...
      event_types: ['*'],
      handler: async (event: HydiEvent) => {
        await this.processEvent(event)
      },
    })
  }

  private async processEvent(event: HydiEvent): Promise<void> {
    ...
    const riskAssessment: RiskAssessment = this.riskEngine.assess(event)
  ...
  private isRedundantTask(event: HydiEvent): boolean {
    for (const [, activeTask] of this.activeTasks.entries()) {
      if (
        activeTask.task_type === event.type &&
  ...
  private createDirective(event: HydiEvent): HeidiDirective {
    const routingAgents = this.taskRoutingMatrix.get(event.type) ?? []
    const primaryAgent = routingAgents[0] ?? 'unknown'

    return {
      task_id: `task_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`,
  ...
  private validateAgentAvailability(agents: string[]): string[] {
    ...
    if (available.length === 0) {
      console.warn(`[HEIDI] No available agents for: ${agents.join(', ')}`)
      return agents
    }
    return available
  }
  ...
  private async routeToAgent(agentId: string, directive: HeidiDirective): Promise<void> {
    ...
    try {
      ...
      console.log(`[HEIDI] Routed ${directive.task_type} → ${agentId} (${directive.priority})`)
    } catch (err) {
      console.error(`[HEIDI] Task execution failed: ${directive.task_id}`, err)
  ...
  private generateTaskDescription(event: HydiEvent): string {
  ...
  private defineExpectedOutcome(event: HydiEvent): string {
  ...
  private assessTaskRisk(event: HydiEvent): 'low' | 'medium' | 'high' | 'critical' {
  ...
  private async escalateForApproval(event: HydiEvent, riskAssessment: RiskAssessment): Promise<void> {
    const approvalRequest = {
      decision_id: `approval_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`,
  ...
  private emit(eventType: string, payload: unknown): void {
    // Publish internal escalation events through the event bus
  ...
  private updateMetrics(): void {
    const total = this.systemMetrics.tasks_processed
    const done = this.systemMetrics.tasks_completed
    this.systemMetrics.system_efficiency = total > 0 ? done / total : 0
  ...
  getSystemStatus(): HeidiSystemStatus {
    return {
      ...
      agent_status: this.agentRegistry.getAllAgents(),
      routing_matrix: Object.fromEntries(this.taskRoutingMatrix),
    }
  }
  ...
  completeTask(taskId: string): void {
    ...
    if (task) {
      ...
      console.log(`[HEIDI] Task completed: ${task.task_type}`)
    }
  }
  ...
  async start(): Promise<void> {
    ...
    while (this.running) {
      try {
        ...
        console.error('[HEIDI] Error in main loop:', err)
      }
    }
  }
