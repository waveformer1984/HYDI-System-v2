...
export class FinanceAgent extends BaseAgent {
  ...
  private async handleExpenseApproved(event: any): Promise<void> {
    ...
    if (amount > approvalThreshold) { await this.triggerHighValueAlert(amount, category); }
  ...
  private async processExpense(amount: number, category: string, description: string, accountId: string, metadata: any): Promise<void> {
    ...
    if (account) { account.balance -= amount; account.transactionHistory.push(transaction); }
    this.treasury.cashOnHand -= amount;
    this.treasury.totalAssets -= amount;
    const spendingLimit = this.controls.spendingLimits.get(category);
    if (spendingLimit) { spendingLimit.currentSpend += amount; spendingLimit.remainingSpend = spendingLimit.limit - spendingLimit.currentSpend; }
    ...
    this.emit_event('FINANCIAL_UPDATE', { type: 'expense_processed', amount, category, description, account_balance: account?.balance, cash_position: this.treasury.cashOnHand, timestamp: new Date().toISOString() }, 'broadcast', 'medium');
  }
  ...
  private async handleRevenueReceived(event: any): Promise<void> {
    ...
    if (this.metrics.runway > this.controls.alertThresholds.runwayWarning) { await this.resolveRunwayAlerts(); }
  ...
  private async handleBudgetRequest(event: any): Promise<void> {
    ...
    if (this.budgets.has(budgetId)) { throw new Error(`Budget ${budgetId} already exists`); }
    const budget: Budget = { id: budgetId, name, period, totalAmount, allocatedAmount: totalAmount, spentAmount: 0, remainingAmount: totalAmount, categories: new Map(), startDate, endDate, status: 'active', alerts: [] };
    categories.forEach((cat: any) => {
      budget.categories.set(cat.name, { name: cat.name, allocatedAmount: cat.amount, spentAmount: 0, remainingAmount: cat.amount, percentage: (cat.amount / totalAmount) * 100, transactions: [], alerts: [] });
    });
    this.budgets.set(budgetId, budget);
    this.emit_event('BUDGET_CREATED', { budgetId, name, period, totalAmount, categories: Array.from(budget.categories.entries()), status: 'active', created_by: this.id, timestamp: new Date().toISOString() }, 'broadcast', 'medium');
  ...
  private async handleFinancialStatusRequest(event: any): Promise<void> {
    ...
    const status = { treasury: this.treasury, budgets: Array.from(this.budgets.values()), cashFlow: this.cashFlow, forecasts: Array.from(this.forecasts.values()), metrics: this.metrics, controls: this.controls, alerts: this.alerts, runway: this.metrics.runway, burnRate: this.metrics.burnRate, timestamp: new Date().toISOString() };
    this.emit_event('FINANCIAL_STATUS_REPORT', { status, generated_by: this.id, timestamp: new Date().toISOString() }, event.source || 'broadcast', 'medium');
  ...
  private async handleForecastRequest(event: any): Promise<void> {
    ...
    this.emit_event('FINANCIAL_FORECAST', { forecast, generated_by: this.id, timestamp: new Date().toISOString() }, 'broadcast', 'medium');
  ...
  private async generateForecast(type: string, horizon: number, confidence: number): Promise<Forecast> {
    ...
    if (type === 'cash_flow') {
      ...
      for (let day = 1; day <= horizon; day++) {
        ...
        predictions.push({ date: date.toISOString(), predicted, confidence: confidence * (1 - day / horizon), upperBound: predicted * 1.2, lowerBound: predicted * 0.8 });
      }
    } else if (type === 'runway') {
      assumptions.push(
      ...
      for (let day = 1; day <= horizon; day++) {
        ...
        predictions.push({ date: date.toISOString(), predicted, confidence, upperBound: predicted * 1.5, lowerBound: predicted * 0.5 });
      }
    }
    
    return { id: forecastId, type: type as any, period: 'daily', horizon, confidence, predictions, assumptions, scenarios: [], lastUpdated: new Date().toISOString() };
  }

  private predictCashFlow(_date: Date, assumptions: ForecastAssumption[]): number {
    ...
    return ((monthlyRevenue / 30) * seasonalAdjustment) - (monthlyExpenses / 30);
  }
  ...
  private async handleSpendingLimitUpdate(event: any): Promise<void> {
    ...
    if (spendingLimit) {
      ...
      this.emit_event('SPENDING_LIMIT_UPDATED', { category, limit, period, currentSpend: spendingLimit.currentSpend, remainingSpend: spendingLimit.remainingSpend, updated_by: this.id, timestamp: new Date().toISOString() }, 'broadcast', 'medium');
    }
  }
  ...
  private async handleReserveAdjustment(event: any): Promise<void> {
    ...
    if (type === 'emergency_fund') { this.treasury.emergencyFund += amount; this.treasury.cashOnHand -= amount; }
    else if (type === 'reserve_fund') { this.treasury.reserveFund += amount; this.treasury.cashOnHand -= amount; }
    else if (type === 'investment_fund') { this.treasury.investmentFund += amount; this.treasury.cashOnHand -= amount; }
    this.treasury.lastUpdated = new Date().toISOString();
    this.emit_event('RESERVE_ADJUSTED', { type, amount, reason, new_balances: { emergency_fund: this.treasury.emergencyFund, reserve_fund: this.treasury.reserveFund, investment_fund: this.treasury.investmentFund, cash_on_hand: this.treasury.cashOnHand }, adjusted_by: this.id, timestamp: new Date().toISOString() }, 'broadcast', 'medium');
  }
  ...
  private calculateBurnRate(): number { return this.metrics.totalExpenses / 30; }
  ...
  private calculateBudgetUtilization(): number {
    ...
    for (const budget of this.budgets.values()) { totalAllocated += budget.allocatedAmount; totalSpent += budget.spentAmount; }
  ...
  private async checkFinancialAlerts(): Promise<void> {
    ...
    if (this.metrics.runway < this.controls.alertThresholds.runwayCritical) {
      alerts.push({ id: `runway_critical_${Date.now()}`, type: 'runway', severity: 'critical', title: 'Critical Runway Alert', message: `Runway is ${this.metrics.runway} days - immediate action required`, timestamp: new Date().toISOString(), resolved: false, actionRequired: true, recommendedActions: ['Seek immediate funding', 'Reduce non-essential expenses', 'Consider emergency measures'], data: { runway: this.metrics.runway, burnRate: this.metrics.burnRate } });
    } else if (this.metrics.runway < this.controls.alertThresholds.runwayWarning) {
      alerts.push({ id: `runway_warning_${Date.now()}`, type: 'runway', severity: 'high', title: 'Runway Warning', message: `Runway is ${this.metrics.runway} days - monitor closely`, timestamp: new Date().toISOString(), resolved: false, actionRequired: true, recommendedActions: ['Monitor spending trends', 'Plan funding strategies', 'Review budget allocations'], data: { runway: this.metrics.runway, burnRate: this.metrics.burnRate } });
    }
    if (this.treasury.cashOnHand < this.controls.reserveRequirements.minimumCashBalance) {
      alerts.push({ id: `cash_critical_${Date.now()}`, type: 'cash_flow', severity: 'critical', title: 'Critical Cash Position', message: `Cash balance $${this.treasury.cashOnHand} below minimum $${this.controls.reserveRequirements.minimumCashBalance}`, timestamp: new Date().toISOString(), resolved: false, actionRequired: true, recommendedActions: ['Transfer from reserves', 'Seek immediate funding', 'Reduce expenses'], data: { cashPosition: this.treasury.cashOnHand, minimumBalance: this.controls.reserveRequirements.minimumCashBalance } });
    }
    ...
    if (totalReserves < reserveTarget * this.controls.alertThresholds.reserveDepletion) {
      alerts.push({ id: `reserve_depletion_${Date.now()}`, type: 'cash_flow', severity: 'medium', title: 'Reserve Depletion Warning', message: `Reserves at ${totalReserves} below target of ${reserveTarget}`, timestamp: new Date().toISOString(), resolved: false, actionRequired: true, recommendedActions: ['Replenish reserves', 'Review reserve allocation', 'Consider reserve requirements'], data: { totalReserves, reserveTarget, depletionRatio: totalReserves / reserveTarget } });
    }
    if (this.metrics.budgetUtilization > this.controls.alertThresholds.budgetExceedance * 100) {
      alerts.push({ id: `budget_exceeded_${Date.now()}`, type: 'budget', severity: 'medium', title: 'Budget Utilization Alert', message: `Budget utilization at ${this.metrics.budgetUtilization.toFixed(1)}% exceeds threshold`, timestamp: new Date().toISOString(), resolved: false, actionRequired: true, recommendedActions: ['Review budget allocations', 'Consider budget reallocation', 'Implement spending controls'], data: { utilization: this.metrics.budgetUtilization, threshold: this.controls.alertThresholds.budgetExceedance } });
    }
    this.alerts = alerts;
    for (const alert of alerts) { this.emit_event('FINANCIAL_ALERT', alert, 'broadcast', alert.severity); }
    if (alerts.length > 0) console.log(`[Finance Agent] Generated ${alerts.length} financial alerts`);
  }

  private async triggerSpendingLimitAlert(spendingLimit: SpendingLimit, amount: number, category: string): Promise<void> {
    const alert: FinancialAlert = { id: `spending_limit_${category}_${Date.now()}`, type: 'expense', severity: 'high', title: 'Spending Limit Exceeded', message: `Spending limit for ${category} exceeded by $${amount - spendingLimit.limit}`, timestamp: new Date().toISOString(), resolved: false, actionRequired: true, recommendedActions: ['Review spending priorities', 'Increase spending limit', 'Defer non-essential expenses'], data: { category, limit: spendingLimit.limit, currentSpend: spendingLimit.currentSpend, attemptedAmount: amount } };
  ...
  private async triggerHighValueAlert(amount: number, category: string): Promise<void> {
    const alert: FinancialAlert = { id: `high_value_${category}_${Date.now()}`, type: 'expense', severity: 'medium', title: 'High Value Expense Alert', message: `High value expense of $${amount} in ${category} requires review`, timestamp: new Date().toISOString(), resolved: false, actionRequired: true, recommendedActions: ['Review expense justification', 'Check approval process', 'Consider alternative options'], data: { amount, category } };
    this.alerts.push(alert);
  ...
  private async resolveRunwayAlerts(): Promise<void> {
    ...
    for (const alert of runwayAlerts) { alert.resolved = true; alert.resolvedAt = new Date().toISOString(); }
    if (runwayAlerts.length > 0) { this.emit_event('FINANCIAL_ALERTS_RESOLVED', { resolved_alerts: runwayAlerts, timestamp: new Date().toISOString() }, 'broadcast', 'low'); }
  }

  private async updateForecasts(): Promise<void> {
    for (const forecast of this.forecasts.values()) { forecast.lastUpdated = new Date().toISOString(); }
  }

  public getFinancialSystemStatus(): any {
    return { treasury: this.treasury, budgets: Array.from(this.budgets.values()), cashFlow: this.cashFlow, forecasts: Array.from(this.forecasts.values()), metrics: this.metrics, controls: this.controls, alerts: this.alerts, accounts: Array.from(this.treasury.accounts.values()), spendingLimits: Array.from(this.controls.spendingLimits.values()) };
  }
  ...
  public createFinancialReport(_type: string): any {
    return { type: _type, generated_at: new Date().toISOString(), generated_by: this.id, treasury: this.treasury, metrics: this.metrics, alerts: this.alerts, recommendations: this.generateRecommendations(), summary: this.generateSummary() };
  }

  private generateRecommendations(): string[] {
    const recommendations: string[] = [];
    if (this.metrics.runway < 30) recommendations.push('Seek immediate funding to extend runway beyond 30 days');
    else if (this.metrics.runway < 60) recommendations.push('Plan funding strategies to maintain healthy runway');
    if (this.treasury.cashOnHand < this.controls.reserveRequirements.minimumCashBalance) recommendations.push('Replenish cash position to meet minimum balance requirements');
    if (this.metrics.budgetUtilization > 90) recommendations.push('Review and adjust budget allocations to prevent overspending');
    const totalReserves = this.treasury.emergencyFund + this.treasury.reserveFund;
    const reserveTarget = this.controls.reserveRequirements.reserveTarget;
    if (totalReserves < reserveTarget) recommendations.push('Build up reserves to meet target of $' + reserveTarget.toLocaleString());
    return recommendations;
  }

  private generateSummary(): any {
    return { total_assets: this.treasury.totalAssets, cash_position: this.treasury.cashOnHand, total_reserves: this.treasury.emergencyFund + this.treasury.reserveFund, runway_days: this.metrics.runway, burn_rate: this.metrics.burnRate, budget_utilization: this.metrics.budgetUtilization, net_income: this.metrics.netIncome, profit_margin: this.metrics.profitMargin, active_alerts: this.alerts.filter(a => !a.resolved).length };
  }
}
