...
export class RevenueAgent extends BaseAgent {
  ...
  private evaluateRevenueOpportunity(payload: any): any {
    ...
    const addressableMarket = payload.addressable_market || marketSize * 0.1; // Assume 10% addressable
    const penetrationRate = payload.penetration_rate || 0.05; // 5% penetration assumption
    const revenuePerCustomer = payload.revenue_per_customer || Math.random() * 1000 + 100; // $100-1100
    ...
    const implementationCost = payload.implementation_cost || Math.random() * 50000; // $0-50k
    const timeToRevenue = payload.time_to_revenue || Math.random() * 12 + 3; // 3-15 months
  ...
  private generateFinancialFlowInsights(analysis: any): string[] {
    ...
    analysis.revenue_streams.forEach((stream: any) => {
      totalRevenue += stream.monthly_revenue;
    });
    
    analysis.expense_categories.forEach((expense: any) => {
    ...
    if (growingStreams > 0) {
      insights.push(`${growingStreams} revenue streams showing growth`);
    }
    ...
    if (decliningStreams > 0) {
      insights.push(`${decliningStreams} revenue streams declining - investigate causes`);
    }
    ...
    if (increasingExpenses > 0) {
      insights.push(`${increasingExpenses} expense categories increasing - review for optimization`);
    }
    ...
    if (lowReliabilityStreams > 0) {
      insights.push(`${lowReliabilityStreams} revenue streams have low reliability - consider diversification`);
    }
