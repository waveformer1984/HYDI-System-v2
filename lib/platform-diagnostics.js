...
async function getRuntimeInventory() {
  const components = await Promise.all(REGISTRY.map(checkComponent));
  const canonical = components.filter(c => !c.path.startsWith('modules/') && !c.path.startsWith('keeper/') && !c.path.startsWith('lib/replay-engine') && !c.path.startsWith('lib/protoforge/replay-engine'));
  const legacy = components.filter(c => c.path.startsWith('modules/') || c.path.startsWith('keeper/') || c.path.startsWith('lib/replay-engine') || c.path.startsWith('lib/protoforge/replay-engine'));
  const deprecated = legacy;
