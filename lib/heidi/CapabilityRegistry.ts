...
export const DEFAULT_CAPABILITIES: Array<Omit<CapabilityDescriptor, 'status' | 'healthNote'>> = [
  ...
  {
    capabilityId: 'tool.cancel_task',
    capabilityName: 'Cancel Task',
    description: 'Delete a still-pending task row — the inverse of create_task',
    provider: 'action_executor',
    riskLevel: 'R1',
    autonomyRequirement: 2,
    dependencies: ['tool.create_task'],
    verificationStrategy: 'Verify the task row is absent from the actions table',
    reversible: false,
    timeoutMs: 10000,
    metadata: { actionType: 'cancel_task', inverseOf: 'tool.create_task' },
  },
  {
