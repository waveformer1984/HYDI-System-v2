 ...
 *
 * Not yet wired into live ingress (protoforge-core's POST /cascade/event
 * still uses modules/cascade-complete-v2); see ROADMAP.md.
 */
...
function createPipeline(options = {}) {
  ...
  const classifier = new CascadeClassificationV2();
  const recent = [];
  ...
  /**
   * Run one gateway-format envelope ({ eventId, eventType, source, payload,
   * version?, timestamp? }) through all six stages. Never throws; failures
   * are recorded on the returned trace.
   */
  async function run(envelope) {
    ...
    const ingestion = await runStage(trace, next(), async () => {
      ...
      return { status: 'ok', event_type: envelope.eventType, source: envelope.source };
    });
