 ...
 *   gte, lte, gt, lt, eq, neq, in, nin, contains, startsWith, regex
 *
 * Multi-condition grouping (inside rule.if, alongside or instead of plain
 * field comparisons):
 *   all: [condition, ...]  — every sub-condition must match (AND)
 *   any: [condition, ...]  — at least one sub-condition must match (OR)
 * `all`/`any` nest arbitrarily (each array entry is itself a full condition
 * object, which may contain its own `all`/`any`). `all`/`any` are reserved
 * condition keys — a hypothesis field literally named `all` or `any` cannot
 * be matched via the plain field-comparison form.
 *
...
// Compiled-regex cache, keyed by pattern string — rule.if.regex conditions
// are re-evaluated on every hypothesis, so avoid recompiling per call.
...
const OPS = {
  ...
  regex:      (a, b) => _compileRegex(String(b)).test(String(a)),
};

/**
 * Evaluate a single condition map against a hypothesis.
 * condition: { confidence: { gte: 0.85 }, risk: { lte: 0.3 }, ... }
 * All fields must pass (AND semantics). `all`/`any` keys hold arrays of
 * nested condition objects for explicit grouping — see file header.
 */
function matchCondition(condition, hyp) {
  for (const [key, value] of Object.entries(condition)) {
    if (key === 'all') {
      if (!Array.isArray(value)) throw new Error("DSL 'all' must be an array of conditions");
      if (!value.every((sub) => matchCondition(sub, hyp))) return false;
      continue;
    }
    if (key === 'any') {
      if (!Array.isArray(value)) throw new Error("DSL 'any' must be an array of conditions");
      if (!value.some((sub) => matchCondition(sub, hyp))) return false;
      continue;
    }
        ...
        async (payload) => {
          ...
          for (const cb of this._reloadCallbacks) {
            try {
              ...
              // Isolate callback failures so one bad subscriber can't break hot-reload for the rest.
