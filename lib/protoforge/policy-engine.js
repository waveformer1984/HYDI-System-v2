...
const logger = require('../structured-logger').child({ component: 'ProtoForge' });
...
class PolicyEngine {
  ...
  async recordDecision(decisionObj) {
    ...
    if (error) {
      logger.error('Failed to record decision', { error: error.message });
      return null;
    }
  ...
  async recordOutcome(decisionId, outcome, detail = {}) {
    ...
    if (error) {
      logger.error('Failed to record outcome', { error: error.message });
    }
  }
  ...
  async _loadPolicy(stream) {
    ...
    if (policy) {
      this._policy = policy;
      logger.info('Policy loaded', { name: policy.name, version: policy.version, stream: stream || 'global' });
    } else {
      this._policy = null;
      logger.warn('No active policy found. Failing closed.', { stream: stream || null });
    }
  }
    ...
    this._channel = this._client
        ...
        async (payload) => {
          ...
          for (const cb of this._reloadCallbacks) {
            try {
              ...
              logger.warn('Reload callback failed', { error: err.message });
            }
          }
        }
      )
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          logger.info('Realtime hot-reload active');
        } else if (status === 'CHANNEL_ERROR') {
          logger.warn('Realtime subscription error — hot-reload degraded');
        }
      });
  }
...
async function recordOutcome(decisionId, outcome, detail = {}) {
  ...
  if (!url || !key) {
    logger.warn('Cannot record outcome — Supabase env vars missing');
    return;
  }
