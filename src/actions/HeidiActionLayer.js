...
class HeidiActionLayer extends EventEmitter {
  constructor(config = {}) {
    ...
    this.actionHistory = [];
    this.revenue = { generated: 0, pending: 0, failed: 0, lastRevenue: null };
  ...
  registerAction(name, handler) {
    this.actions.set(name, { name, handler, usage: 0, success: 0, failure: 0, avgLatency: 0 });
  }
  ...
  async executeAction(actionType, params, context = {}) {
    ...
    try {
      ...
      if (result.revenue) this.trackRevenue(result.revenue);
      
      this.actionHistory.push({
        id: actionId, type: actionType, params, context, result,
        latency, success: true, timestamp: new Date().toISOString()
      });
      ...
      this.emit('action_completed', { actionId, actionType, result, latency, context });
      console.log(`[ACTION LAYER] Action completed: ${actionId} (${latency}ms)`);
      
      return { success: true, actionId, result, latency };
      ...
      this.actionHistory.push({
        id: actionId, type: actionType, params, context,
        error: error.message, latency, success: false,
        timestamp: new Date().toISOString()
      });
      
      this.emit('action_failed', { actionId, actionType, error: error.message, latency, context });
      console.error(`[ACTION LAYER] Action failed: ${actionId} - ${error.message}`);

      // Non-blocking self-healing: ask Claude to diagnose and suggest a retry strategy
      try {
        const healer = require('../healing/SelfHealingService');
        healer.healFromCrash(
  ...
  async executeStripePayment(params, context) {
    if (!this.config.enableRevenueActions) throw new Error('Revenue actions are disabled');
    if (!this.config.stripeSecretKey) throw new Error('Stripe secret key not configured');
  ...
  async createCheckout(params, context) {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: params.currency || 'usd',
          product_data: { name: params.productName, description: params.description, images: params.images || [] },
    ...
    await this.storePaymentEvent({ type: 'checkout_created', sessionId: session.id, amount: params.price, metadata: params.metadata });
    return { sessionId: session.id, url: session.url, revenue: 0 };
  }
  ...
  async refundPayment(params, context) {
    ...
    await this.storePaymentEvent({
      type: 'refund_processed', refundId: refund.id,
      amount: -params.amount, paymentIntentId: params.paymentIntentId, metadata: params.metadata
    });
    return { refund, status: refund.status, revenue: -params.amount };
  }
  ...
  async updateSubscription(params, context) {
    ...
    await this.storePaymentEvent({
      type: 'subscription_updated', subscriptionId: subscription.id,
      status: subscription.status, metadata: params.metadata
    });
    return { subscription, status: subscription.status, revenue: 0 };
  }
  
  async sendEmail(params, context) {
    if (!this.config.emailApiKey) throw new Error('Email API key not configured');
    console.log(`[EMAIL] Sending to ${params.to}: ${params.subject}`);
    
    let result;
    switch (this.config.emailProvider) {
      case 'resend':   result = await this.sendResendEmail(params); break;
      case 'sendgrid': result = await this.sendSendGridEmail(params); break;
      default: throw new Error(`Unknown email provider: ${this.config.emailProvider}`);
    }
    
    await this.storeCommunicationEvent({
      type: 'email_sent', provider: this.config.emailProvider,
      to: params.to, subject: params.subject, messageId: result.id
    });
    return { messageId: result.id, status: 'sent', provider: this.config.emailProvider };
  }
  ...
  async sendSendGridEmail(params) {
    ...
    const result = await sgMail.send({
      to: params.to, from: params.from || 'noreply@hydi.system',
  ...
  async sendWebhook(params, context) {
    ...
    const response = await fetch(params.url, {
      ...
      headers: { 'Content-Type': 'application/json', 'User-Agent': 'Heidi-ActionLayer/1.0', ...params.headers },
      body: JSON.stringify(params.data)
    });
    if (!response.ok) throw new Error(`Webhook failed with status ${response.status}`);
    const responseData = await response.json();
    await this.storeCommunicationEvent({ type: 'webhook_sent', url: params.url, method: params.method, status: response.status, responseData });
    return { status: response.status, data: responseData, success: true };
  }
  ...
  async updateDatabase(params, context) {
    ...
    switch (params.operation) {
      case 'insert': result = await this.insertRecord(params.table, params.record); break;
      case 'update': result = await this.updateRecord(params.table, params.record, params.condition); break;
      case 'delete': result = await this.deleteRecord(params.table, params.condition); break;
      case 'upsert': result = await this.upsertRecord(params.table, params.record, params.conflictColumns); break;
      default: throw new Error(`Unknown database operation: ${params.operation}`);
    }
    await this.storeSystemEvent({ type: 'database_updated', table: params.table, operation: params.operation, recordId: result.id || result[0]?.id });
    return result;
  }
  ...
  async updateRecord(table, record, condition) {
    const { data, error } = await supabase.from(table).update(record).match(condition).select();
    if (error) throw error;
    return data;
  }
  ...
  async upsertRecord(table, record, conflictColumns) {
    const { data, error } = await supabase.from(table).upsert(record, { onConflict: conflictColumns }).select();
    if (error) throw error;
    return data;
  }
  
  async launchScript(params, context) {
    if (!this.config.enableScriptExecution) throw new Error('Script execution is disabled');
    ...
    await this.storeSystemEvent({ type: 'script_executed', script: params.script, exitCode: result.exitCode, duration: result.duration });
    return result;
  }
  ...
  async executeScript(scriptPath, args = [], env = {}) {
    return new Promise((resolve, reject) => {
      const startTime = Date.now();
      const child = spawn('node', [scriptPath, ...args], { env: { ...process.env, ...env }, stdio: 'pipe' });
      let stdout = '';
      let stderr = '';
      child.stdout.on('data', (data) => { stdout += data.toString(); });
      child.stderr.on('data', (data) => { stderr += data.toString(); });
      child.on('close', (code) => { resolve({ exitCode: code, stdout, stderr, duration: Date.now() - startTime, success: code === 0 }); });
  ...
  async generateOffer(params, context) {
    ...
    const offer = {
      ...
      currency: params.currency || 'usd',
      validUntil: params.validUntil || new Date(Date.now() + 86400000).toISOString(),
      terms: params.terms || this.generateOfferTerms(params.type),
      metadata: params.metadata || {}
    };
    await this.storeOffer(offer);
    await this.storeSystemEvent({ type: 'offer_generated', offerId: offer.id, offerType: params.type, price: offer.price });
    return { offer, revenue: 0 };
  }
  ...
  calculateOfferPrice(type) {
    const prices = { 'starter': 29, 'pro': 99, 'enterprise': 299, 'custom': 0 };
  ...
  async deployPage(params, context) {
    ...
    const deployment = {
      ...
      pageId: params.pageId, content: params.content,
    ...
    await fs.writeFile(path.join(deployDir, `${params.pageId}.html`), params.content);
    await this.storeDeployment(deployment);
    await this.storeSystemEvent({ type: 'page_deployed', pageId: params.pageId, deploymentId: deployment.id, environment: deployment.environment, url: deployment.url });
    return { deployment, url: deployment.url, revenue: 0 };
  }
  ...
  async storePaymentEvent(event) {
    try {
      const { error } = await supabase.from('payment_events').insert({
        ...
        event_type: event.type, payment_data: event, timestamp: new Date().toISOString()
      });
      if (error) throw error;
    } catch (error) {
  ...
  async storeCommunicationEvent(event) {
    try {
      const { error } = await supabase.from('communication_events').insert({
        ...
        event_type: event.type, communication_data: event, timestamp: new Date().toISOString()
      });
      if (error) throw error;
    } catch (error) {
  ...
  async storeSystemEvent(event) {
    try {
      const { error } = await supabase.from('system_events').insert({
        ...
        event_type: event.type, system_data: event, timestamp: new Date().toISOString()
      });
      if (error) throw error;
    } catch (error) {
  ...
  async storeOffer(offer) {
    try {
      const { error } = await supabase.from('offers').insert({ offer_id: offer.id, offer_data: offer, created_at: new Date().toISOString() });
      if (error) throw error;
    } catch (error) {
  ...
  async storeDeployment(deployment) {
    try {
      const { error } = await supabase.from('deployments').insert({ deployment_id: deployment.id, deployment_data: deployment, created_at: new Date().toISOString() });
      if (error) throw error;
    } catch (error) {
  ...
  trackRevenue(amount) {
    if (amount > 0) {
      ...
      this.revenue.lastRevenue = { amount, timestamp: Date.now() };
    } else if (amount < 0) {
      this.revenue.failed += Math.abs(amount);
    } else {
      this.revenue.pending += 1;
    }
    this.emit('revenue_tracked', { amount, total: this.revenue.generated, pending: this.revenue.pending, failed: this.revenue.failed });
  }
  ...
  getStatus() {
    return {
      ...
      config: { maxConcurrent: this.config.maxConcurrentActions, revenueEnabled: this.config.enableRevenueActions, scriptsEnabled: this.config.enableScriptExecution }
    };
  }
  ...
  getActionStats() {
    ...
    for (const [name, action] of this.actions) {
      stats[name] = {
        usage: action.usage, success: action.success, failure: action.failure,
  ...
  async reset() {
    ...
    for (const action of this.actions.values()) {
      action.usage = 0; action.success = 0; action.failure = 0; action.avgLatency = 0;
    }
