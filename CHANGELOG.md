...
- Phase 6, milestone 1 — Autonomous Task Engine: added `src/hydi-v3/TaskEngine.js`, a persistent, dependency-aware task queue with priority ordering, DAG execution, atomic JSON persistence, lifecycle-safe `start`/`stop`/`destroy`, rollback compensation, and resumable state. Added `tests/unit/hydi-v3/TaskEngine.test.js` (11 tests) covering enqueue/priority/dependencies, failure blocking, rollback, persistence, and a 50-task benchmark under one second.
