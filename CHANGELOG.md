...
- Mobile-ops Realtime bridge: `api/events/stream.js` now subscribes once per process to Supabase Realtime `postgres_changes` on `agent_control_commands` and `notifications`, re-emitting them onto the shared event bus. Fixes a real gap — `workers/WorkerOrchestrator.js` runs as its own process (see `ecosystem.config.js`), so its local `publish()` calls never reached a phone's SSE connection in the Next.js process; a completed worker command previously only surfaced on the next 30s REST poll, and `createNotification()` never published at all despite the mobile client already listening for a `notification` event. New `command_result` SSE event type; `hydi-mobile-protoforge.html`'s Ops tab refreshes worker cards on it. `supabase/migrations/20260722120000_realtime_mobile_ops_bridge.sql` adds both tables to the `supabase_realtime` publication. See `docs/MOBILE_OPERATIONS.md`.
- `hydi-worker-orchestrator` PM2 app in `ecosystem.config.js` — `workers/WorkerOrchestrator.js` (the mobile-ops command-queue consumer) wasn't process-managed anywhere, so queued worker start/stop/restart/scale commands were accepted but never executed.
- Root `icons/icon-192.png`, `icon-512.png`, `icon.svg` (copied from the existing `docs/icons/` set) — `manifest.json`'s Capacitor build referenced these but they didn't exist in the repo.
