...
-- Required for: 20260424145921_hydi_stripe_sync_function.sql
...
DROP POLICY IF EXISTS "Event bus events are viewable by authenticated users" ON public.event_bus_events;
CREATE POLICY "Event bus events are viewable by authenticated users" ON public.event_bus_events
    FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Event bus events are manageable by service role" ON public.event_bus_events;
