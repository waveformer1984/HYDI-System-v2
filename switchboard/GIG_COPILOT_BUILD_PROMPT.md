# Switchboard ΓÇö IDE Build Prompt
...
platforms or bypass their rules ΓÇö it is a decision-support and workflow
...
4. Secure, device-locked access ΓÇö approved devices only.
...
10. Feel like a dispatch console ΓÇö operational, not a toy or social app.
...
- If no official API exists for a platform, use manual entry or permitted
  integrations only ΓÇö never reverse-engineered automation.
...
- Visual style: dark, high-contrast, clean, mobile-first, minimal clutter,
  ...
  or playful styling ΓÇö it should feel like a control room, not an app for
  fun.
...
2. QR Pairing & Access
   ...
   - Encrypt stored access data; allow revocation of any device/session at
     ...
     role changes.
   - QR pairing grants Switchboard access only ΓÇö never parent-app
     credential sharing.
...
6. Tag-Team Mode
   ...
   - Hard rule for Favor and similar apps: one live session, one device,
     one lead user, one session owner ΓÇö always.
...
9. Savings Modules
   ...
   - Insurance: back-burner savings and risk module ΓÇö starts as a policy
