/**
 * diagnose-toolcall-2.js
 *
 * Isolates which factor breaks tool-calling for llama3.2 via Ollama:
 *   A) Full system prompt + ALL 14 tools, temperature 0.7  (production, known broken)
 *   B) Full system prompt + ALL 14 tools, temperature 0.1
 *   C) NO system prompt   + ALL 14 tools, temperature 0.7
 *   D) Full system prompt + ONLY get_ursula_live tool, temperature 0.7
 *   E) Short system prompt (no tool-name listing) + ALL 14 tools, temperature 0.7
 *
 * Run: node diagnose-toolcall-2.js
 */
'use strict';
...
const ONE_TOOL = [TOOLS[6]]; // get_ursula_live

const FULL_SYSTEM_PROMPT = `You are Heidi, the AI command interface for the ProtoForge ecosystem — an autonomous revenue-generating platform.
...
const SHORT_SYSTEM_PROMPT = `You are Heidi, the AI command interface for the ProtoForge ecosystem. You have real function-calling tools available. When a request requires a tool, you MUST emit an actual tool call — never describe a command in text. Be direct and concise.`;

const USER_MESSAGE = 'Use your get_ursula_live tool with path /api/health to check the live Ursula Vercel app status.';

async function run(label, { systemPrompt, tools, temperature }) {
    const messages = [];
    if (systemPrompt) messages.push({ role: 'system', content: systemPrompt });
    messages.push({ role: 'user', content: USER_MESSAGE });

    const body = { model: MODEL, messages, stream: false, tools, options: { temperature, num_predict: 600 } };
    ...
    try {
        ...
        console.log(`[${label}] tool_calls: ${tc && tc.length ? 'YES (' + tc.map(t=>t.function?.name).join(',') + ')' : 'NO'}`);
        if (!tc || !tc.length) {
            console.log(`         content: ${JSON.stringify((data.message?.content || '').slice(0, 120))}...`);
        }
    } catch (e) {
...
(async () => {
    ...
    await run('A: full prompt + 14 tools + temp 0.7', { systemPrompt: FULL_SYSTEM_PROMPT, tools: TOOLS, temperature: 0.7 });
    await run('B: full prompt + 14 tools + temp 0.1', { systemPrompt: FULL_SYSTEM_PROMPT, tools: TOOLS, temperature: 0.1 });
    await run('C: NO prompt    + 14 tools + temp 0.7', { systemPrompt: null, tools: TOOLS, temperature: 0.7 });
    await run('D: full prompt  + 1 tool   + temp 0.7', { systemPrompt: FULL_SYSTEM_PROMPT, tools: ONE_TOOL, temperature: 0.7 });
    await run('E: SHORT prompt + 14 tools + temp 0.7', { systemPrompt: SHORT_SYSTEM_PROMPT, tools: TOOLS, temperature: 0.7 });
})();
