...
| `integration-tests.yml` | push to `clean-main`, all PRs | `npm run typecheck:hydi-v3`, `npm run lint:hydi-v3`, `npm run test:integration:jest` — the full hermetic operational integration suite (13 suites / 76 tests, ~25s), no credentials or local environment state required |
...
**Integration tests** (`tests/integration/**`, 13 suites / 76 tests as of 2026-09-25): real `OperatorSession`/`HYDIContinuousRuntime` instances exercising the full executive stack end-to-end (temp data directories, no mocked internals). Deliberately excluded from `jest.config.js`'s `testMatch` so they never silently inflate `npm test`'s runtime — run them explicitly:
```bash
