...
- **`src/server.js`**: a separate, standalone Express app (`node src/server.js`, a real `package.json` script — `"server"` — but not listed in this file's Commands section above) with its own ~60 routes. It is **not** the same thing as `heidi-core/index-clean-3458.js`, the process `.ports.json` and `scripts/start-hydi.js` actually treat as "HEIDI Core." Whether `src/server.js` is deployed anywhere real is an open question — see DEPLOYMENT.md §5.
