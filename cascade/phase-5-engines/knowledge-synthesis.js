...
class KnowledgeSynthesisEngine extends EventEmitter {
  ...
  async findRelationship(pattern1, pattern2) {
    try {
      // Check for shared CASCADE concepts
  ...
  classifyRelationship(pattern1, pattern2) {
    const shared = new Set([...pattern1.cascadePath].filter(x => pattern2.cascadePath.includes(x)));

    if (shared.size / Math.max(pattern1.cascadePath.length, pattern2.cascadePath.length) > 0.6) {
