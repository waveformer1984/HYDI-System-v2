...
| `workers/WorkerOrchestrator.js` | Yes | n/a (no HTTP server; polls Supabase) | Worker fleet + `agent_control_commands` command-queue consumer for the mobile Ops tab | **PM2-managed as of 2026-07-22** (app name `hydi-worker-orchestrator`), closing the gap `docs/MOBILE_OPERATIONS.md` flagged as tech debt — previously nothing ran `pollControlCommands()` continuously, so mobile worker start/stop/restart/scale requests were accepted and queued but never executed. Same "can't confirm it's running right now" caveat as the rest of the PM2 fleet. |
