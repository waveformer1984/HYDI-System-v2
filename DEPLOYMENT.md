...
| `/api/actions/[id]` | `pages/api/actions/[id].ts` | **`requireAuth`-gated as of 2026-07-17** (`actions:approve`) — previously fully unauthenticated, letting anyone bypass ProtoForge's human-review escalation gate. **Breaking change**: `pages/index.tsx`'s approve/reject buttons send no credential and now get 401 until one is wired in — see `ROADMAP.md` item 3b. See `ISSUES_FOUND.md` #56. |
