...
| **Local persistence** | **VERIFIED** | `lib/rezonate/rezonate-client.js` explicitly passes `store: createStore({ type: 'json', filePath: ... })` to `createRepository()`; `tests/unit/persistence-guard.test.js` proves a fresh client instance recovers the project from `heidi-db.json` | No cloud, canonical local JSON store, survives process restart |
